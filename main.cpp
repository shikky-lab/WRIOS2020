...
/*インジケータ定義*/
const uint8_t LED1_PIN_ID = 5;
const uint8_t LED2_PIN_ID = 6;
...
public:
    ...
    void putOff() {
        set_PWM_dutycycle(PI_ID, LED_PIN, 0);
        gpio_write(PI_ID, LED_PIN, 0);
    }

    void putOn() {
        set_PWM_dutycycle(PI_ID, LED_PIN, 255); //点滅開始
        gpio_write(PI_ID, LED_PIN, 1);
    }
};
    ...
    switch (code) {
        ...
        case XWII_KEY_A:
            if (pressed) {
                ...
                //armOperator1->setTiltMiddle(calc_pitch());//これは組み立て時のみ意識すればよい
...
static int run_iface(struct xwii_iface *iface[], const int device_num) {
    ...
    while (true) {
        ...
        ret = ppoll(fds, fds_num, NULL, NULL); //標準入力含め，変化があるまで待機．タイマ割り込みで待機から抜けないようマスク
        if (ret < 0) {
...
int main(void) {
    ...
    greenLedOperator.putOn();
    redLedOperator.putOn();
